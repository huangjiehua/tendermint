...
// Add 99 txs. Grab chunks to put in block. All the others become invalid because of nonce errors except the last one.
...
func (tp *TestPeer) Send(chID byte, msg interface{}) bool {
	...
	tp.ordered = append(tp.ordered, string(id))
	tp.received += 1
	tp.done <- tp.received
	return true
}
